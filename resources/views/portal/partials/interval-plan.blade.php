@if($interval_plan['enabled'] ?? false)
    <div>
        ...
        <button type="button" id="interval-plan-option" data-plan-option data-plan-type="interval"
            data-plan-price="{{ $interval_plan['price_24h'] * 2 }}" data-plan-duration="24"
            ...
            <span class="min-w-0 flex-1">
                ...
                <span class="block text-xs text-muted">A partir de 2 dias · 24 horas corridas por dia</span>
            </span>
            <span data-plan-price-display class="text-base font-extrabold text-ink whitespace-nowrap">R${{ number_format($interval_plan['price_24h'] * 2, 2, ',', '.') }}</span>
        </button>
    </div>
