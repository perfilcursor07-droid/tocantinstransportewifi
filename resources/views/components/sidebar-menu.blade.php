...
<aside id="sidebar"
    ...
    <nav class="flex-1 px-2 py-3 overflow-y-auto space-y-4">
        ...
        <div>
            ...
            @if(Auth::user()->role === 'admin')
            @php $active = request()->routeIs('admin.driver-pix*'); @endphp
            <a href="{{ route('admin.driver-pix.index') }}" onclick="closeSidebarOnMobile()" title="Pagamentos Motoristas"
               class="sidebar-link flex items-center gap-3 px-2.5 py-2 rounded-xl text-sm font-semibold transition-all mt-0.5
                ...
                <span class="sidebar-label truncate">Pagamentos</span>
            </a>
            @endif
        </div>
