...
(async () => {
    ...
    try {
        for (const width of [390, 1440]) {
            ...
            assert.equal(await page.locator('#interval-end').inputValue(), next);
            await page.locator('[name="interval-hours"][value="24"]').check();
            assert.match(await page.locator('#interval-summary').textContent(), /27,96/);
            await page.locator('#interval-end').fill(start);
            assert.equal(await page.locator('#connect-btn').isDisabled(), true);
            await page.locator('#interval-start').fill(next);
            assert.equal(await page.locator('#connect-btn').isDisabled(), false);
            await page.locator('[name="interval-hours"][value="12"]').check();
            assert.match(await page.locator('#interval-summary').textContent(), /13,98/);
            ...
            console.log(JSON.stringify({ width, checks: 'dates, totals, min two days, standard plan, input bounds', errors }));
            ...
            assert.equal(await page.locator('#plan_interval_price_12h').inputValue(), '6.99');
